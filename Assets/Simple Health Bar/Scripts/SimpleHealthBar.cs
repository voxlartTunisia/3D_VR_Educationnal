...
{	
	...
	public TextLocation location = TextLocation.BEFORE;
	...
	void DisplayTextHandler ()
	{
		...
		switch( displayText )
		{
			case DisplayText.Percentage:{
				if (location == TextLocation.BEFORE)
					barText.text = additionalText + (GetCurrentFraction * 100).ToString("F0") + "%";
				else
					barText.text = (GetCurrentFraction * 100).ToString("F0") + "%" + additionalText;
			}break;
			case DisplayText.CurrentValue:{
				if (location == TextLocation.BEFORE)
					barText.text = additionalText + (GetCurrentFraction * _maxValue).ToString("F0");
				else
					barText.text = (GetCurrentFraction * _maxValue).ToString("F0") + additionalText;
			}break;
			case DisplayText.CurrentAndMaxValues:{
				if (location == TextLocation.BEFORE)
					...
					barText.text = (GetCurrentFraction * _maxValue).ToString("F0") + " / " +
					               _maxValue.ToString() + additionalText;
			}break;
		}
	}
