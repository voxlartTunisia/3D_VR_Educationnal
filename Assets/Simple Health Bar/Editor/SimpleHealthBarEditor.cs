		...
		if( EditorGUILayout.BeginFadeGroup( ImageAssigned.faded ) )
		{
			...
			if( EditorGUILayout.BeginFadeGroup( DisplayTextOption.faded ) )
			{
				...
				EditorGUILayout.PropertyField(location,
					new GUIContent("Additional Text Location", "The location of the additional text"));
				...
				switch( targ.displayText )
				{
					case SimpleHealthBar.DisplayText.Percentage:{
						if (targ.location == SimpleHealthBar.TextLocation.BEFORE)
							EditorGUILayout.LabelField("Text Preview: " + targ.additionalText + testValue + "%");
						else
							EditorGUILayout.LabelField("Text Preview: " + testValue + "%" + targ.additionalText);
					}
					break;
					...
					case SimpleHealthBar.DisplayText.CurrentAndMaxValues:{
						if (targ.location == SimpleHealthBar.TextLocation.BEFORE)
							...
							EditorGUILayout.LabelField("Text Preview: " + testValue + " / 100 " + targ.additionalText);
						}
					}
					break;
					default:
